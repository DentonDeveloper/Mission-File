...
reTrieve = player addaction [_rtv,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVeh select 0);',1,true,true,"",'player distance copsavepoint < 5'];
reTrieve1 = player addaction [_rtv2,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVeh select 1);',1,true,true,"",'player distance copsavepoint < 5'];
reTrieve2 = player addaction [_rtv3,"noscript.sqf",'continue = true; vehicle2Spawn = (INV_SavedVeh select 2);',1,true,true,"",'player distance copsavepoint < 5'];
...
_v = format ["vehicle_%1_%2",player,round(time)];
	...
	', player, round(time), INV_CALL_CREATVEHICLE, getpos copsavespawn, getdir _logic];
